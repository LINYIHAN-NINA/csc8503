...
Win32Window::Win32Window(const WindowInitialisation &winInitInfo)
{
    ...
    position.x = winInitInfo.windowPositionX;
    position.y = winInitInfo.windowPositionY;
    Vector2i requestedSize = size;
    if (fullScreen)
    {
        DEVMODE currentSettings;
        ZeroMemory(&currentSettings, sizeof(DEVMODE));
        currentSettings.dmSize = sizeof(DEVMODE);
        EnumDisplaySettings(NULL, ENUM_CURRENT_SETTINGS, &currentSettings);

        DEVMODE dmScreenSettings;                               // Device Mode
        memset(&dmScreenSettings, 0, sizeof(dmScreenSettings)); // Makes Sure Memory's Cleared

        dmScreenSettings.dmSize = sizeof(dmScreenSettings); // Size Of The Devmode Structure
        dmScreenSettings.dmPelsWidth = requestedSize.x > 0 ? (DWORD)requestedSize.x : currentSettings.dmPelsWidth;
        dmScreenSettings.dmPelsHeight = requestedSize.y > 0 ? (DWORD)requestedSize.y : currentSettings.dmPelsHeight;
        dmScreenSettings.dmBitsPerPel = 32; // Selected Bits Per Pixel
        dmScreenSettings.dmFields = DM_BITSPERPEL | DM_PELSWIDTH | DM_PELSHEIGHT;
        if (winInitInfo.refreshRate > 0)
        {
        ...
        {
            std::cout << __FUNCTION__ << " Requested fullscreen mode unavailable, using desktop resolution instead.\n";
            dmScreenSettings.dmPelsWidth = currentSettings.dmPelsWidth;
            dmScreenSettings.dmPelsHeight = currentSettings.dmPelsHeight;
            dmScreenSettings.dmDisplayFrequency = currentSettings.dmDisplayFrequency;
            dmScreenSettings.dmFields = DM_BITSPERPEL | DM_PELSWIDTH | DM_PELSHEIGHT;
            if (currentSettings.dmDisplayFrequency != 0)
            {
                dmScreenSettings.dmFields |= DM_DISPLAYFREQUENCY;
            }
            if (ChangeDisplaySettings(&dmScreenSettings, CDS_FULLSCREEN) != DISP_CHANGE_SUCCESSFUL)
        ...
        if (fullScreen)
        {
            size.x = (int)dmScreenSettings.dmPelsWidth;
            size.y = (int)dmScreenSettings.dmPelsHeight;
            defaultSize = size;
            position.x = 0;
            position.y = 0;
        }
